...
CONF_THRESHOLD        = 0.3   # Güven skoru eşiği
...
# YOLOv8 pose keypoint indeksleri
KP = {
...
# ── Buffer'lar ───────────────────────────────────────────────────────────────
kp_buffers  = {i: (deque(maxlen=KP_BUFFER), deque(maxlen=KP_BUFFER)) for i in range(17)}
l_ua_hist   = deque(maxlen=ELBOW_HIST)   # üst kol dikey açısı (kalçasız)
...
def get_ua_vertical_angle(shoulder, elbow):
    """
    Üst kolun (omuz→dirsek) DİKEY EKSENE göre açısı.
    Kalça gerekmez — pozisyon ve ölçekten bağımsızdır.
    Biseps curl'de dirsek aşağıda sabit durmalı →
    bu açı küçük (dikey) ve referansa yakın olmalıdır.
    """
    ua_vec = np.array(elbow) - np.array(shoulder)
    return vec_angle(ua_vec, np.array([0.0, 1.0]))  # dikey eksen
...
def get_smooth_kp(kps, confs, idx):
    """Güven filtreli + hareketli ortalama uygulanmış keypoint koordinatı."""
    if confs is not None and confs[idx] < CONF_THRESHOLD:
        return None
    x, y = kps[idx]
    if x == 0 and y == 0:
        return None
...
while cap.isOpened():
    ...
    # ── COUNTDOWN ────────────────────────────────────────────────────────────
        ...
        if remaining > 0:
            ...
            if kps is not None:
                pts = {name: get_smooth_kp(kps, confs, idx) for name, idx in KP.items()}
                # Kalça olmadan da başlanabilir — yalnızca omuz+dirsek+bilek yeterli
                ...
                if all(pts[k] is not None for k in arm_required):
                    ref = {
                        # Dikey eksene göre üst kol açısı — kalça gerekmez
                        "l_ua_vert": get_ua_vertical_angle(pts["l_shoulder"], pts["l_elbow"]),
                        "r_ua_vert": get_ua_vertical_angle(pts["r_shoulder"], pts["r_elbow"]),
                    }
                    if hip_available:
                        ref["l_ua_angle"]   = get_upper_arm_angle(pts["l_shoulder"], pts["l_elbow"], pts["l_hip"])
                        ref["r_ua_angle"]   = get_upper_arm_angle(pts["r_shoulder"], pts["r_elbow"], pts["r_hip"])
    ...
    # ── ACTIVE ───────────────────────────────────────────────────────────────
        ...
        if kps is not None:
            ...
            if arm_ok:
                ...
                # Referans dikey açısına göre üst kol sapması
                ...
                # Gövde uyarısı — yalnızca kalça görünüyorsa
